...
export async function GET(request: NextRequest) {
  try {
    ...
    const sql = includeInactive
      ? 'SELECT * FROM package_classes ORDER BY price'
      : 'SELECT * FROM package_classes WHERE is_active = true ORDER BY price'
    ...
    const statsResult = await query(`
      ...
      FROM user_packages
      GROUP BY package_class_id
    `)

    const stats = Object.fromEntries(
      statsResult.rows.map(row => [row.package_class_id, row])
    )
